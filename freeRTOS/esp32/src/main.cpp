...
static SemaphoreHandle_t semaphore = NULL; // Declare a semaphore handle.
...
void setup()
{
    ...
    BaseType_t ping = xTaskCreate(print_ping, "Ping", configMINIMAL_STACK_SIZE, NULL, 1, NULL);

    // Create the task at priority two for print_pong
    BaseType_t pong = xTaskCreate(print_pong, "Pong", configMINIMAL_STACK_SIZE, NULL, 2, NULL);

    // Create the task at priority three for blinking
    BaseType_t blink = xTaskCreate(blinking, "Blinking", configMINIMAL_STACK_SIZE, NULL, 3, NULL);
